...
export const createGenerateCourseTool = (config: {
    userId?: string;
    sessionId?: string;
    latestStructuredPlan?: LearningPlanWithIds | null;
}) => {
    const { userId, sessionId, latestStructuredPlan } = config;
    const areIdsProvided = userId && sessionId;
...
Be comprehensive - this is used to create course content that feels custom-made for them.`),
            ...
            ...(!areIdsProvided
                ? {
                    userId: z
                        .string()
                        .describe(
                            'The ID of the user generating the course.',
                        ),
                    sessionId: z
                        .string()
                        .describe(
        ...
        execute: async ({
            ...
            const effectiveUserId = userId || inputUserId;
            const effectiveSessionId = sessionId || inputSessionId;

            if (!effectiveUserId || !effectiveSessionId) {
                return {
                    errorMessage: 'User ID and Session ID are required to generate a course.',
